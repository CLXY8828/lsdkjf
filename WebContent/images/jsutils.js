...
function getDateFromIdCardNo(idCardObj){  
...
function trimItems(oFrm) {
	...
	for (i=0; i<oFrm.elements.length; i++) {
		...
		if ((type == "text") || (type == "textarea")) {
			oFrm.elements[i].value = oFrm.elements[i].value.replace(/(^\s*)|(\s*$)/g, "");
		}
	}
}
/********************************** zmx 2005.10.05*************************************/
...
/* 代码和下拉列表互相选择
/********************************** lwj 2006.10.23*************************************/
function inputFindText(in_obj,listname){
	var inputvalue = in_obj.value;
    var listObj = document.getElementById(listname);
    var findTag = false;
    for(var i=0;i<listObj.length;i++){
    	listvalue = listObj.options[i].value;
    	if(listvalue == inputvalue){
    		listObj.options.selectedIndex = i;
    		findTag = true;
    		break;
    	}
    }
...
function listChange(list_obj,inputname){
	var listvalue = list_obj.options[list_obj.options.selectedIndex].value;
    var inputObj = document.getElementById(inputname);
    inputObj.value = listvalue;
}
//查询条件清空
function setSelected(listname,inputvalue){
    var listObj = document.getElementById(listname);
    var findTag = false;
    for(var i=0;i<listObj.length;i++){
    	listvalue = listObj.options[i].value;
    ...
    if(document.getElementById(listname+"_code") != undefined){
    	document.getElementById(listname+"_code").value = inputvalue;
    }	
}
function setCheckbox(resetform,listname,inputvalue){
	var values = inputvalue.split(",");
	for(var i=0;i<values.length;i++){
	 	value = values[i];
	 	with(resetform){
			for(j=0;j<length;j++){   				
				if(elements[j].type=="checkbox" && elements[j].name.indexOf(listname) != -1 && elements[j].value == value){ 
...
function setRadio(resetform,listname,inputvalue){
 	with(resetform){
		for(j=0;j<length;j++){   				
			if(elements[j].type=="radio" && elements[j].name.indexOf(listname) != -1 && elements[j].value == inputvalue){ 
				elements[j].checked=true;   
				break; 
			}   
		}
		...
		with(resetform){
			for(i=0;i<length;i++){   				
				...
				if(elements[i].type=="hidden"){ 
					relationtype = elements[i].getAttribute("relationtype");
					if(relationtype != null && relationtype != ""){					
						if(relationtype == "input" || relationtype == "datetime" ){
							elements[elements[i].getAttribute("relationship")].value=elements[i].getAttribute("value");
						}else if(relationtype == "inputscope" || relationtype == "datetimescope" ){
							values = elements[i].getAttribute("value").split(",");
							if(values.length > 0){
								elements[elements[i].getAttribute("relationship")+"_start"].value=values[0];
							}
							if(values.length > 1){
								elements[elements[i].getAttribute("relationship")+"_end"].value=values[1];
							}							
						}else if(relationtype == "tree"){
							values = elements[i].getAttribute("value").split(",");
							if(values == null || values.length == 0){
								elements[elements[i].getAttribute("relationship")+"_id"].value="";
								elements[elements[i].getAttribute("relationship")+"_code"].value="";
								elements[elements[i].getAttribute("relationship")+"_name"].value="";
							}else{
								if(values.length > 0){
									elements[elements[i].getAttribute("relationship")+"_id"].value=values[0];
								}
								if(values.length > 1){
									elements[elements[i].getAttribute("relationship")+"_code"].value=values[1];
								}
								if(values.length > 2){
									elements[elements[i].getAttribute("relationship")+"_name"].value=values[2];
								}	
							}						
						}else if(relationtype == "select"){
							setSelected(elements[i].getAttribute("relationship"),elements[i].getAttribute("value"));													
						}else if(relationtype == "checkbox"){
							setCheckbox(resetform,elements[i].getAttribute("relationship"),elements[i].getAttribute("value"));													
						}else if(relationtype == "radio"){
							setRadio(resetform,elements[i].getAttribute("relationship"),elements[i].getAttribute("value"));													
						}    
					}					 
				} 		
			}
